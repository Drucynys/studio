  ...
  try {
    ...
    while (hasMore) {
      try {
        ...
        logs.push(
          `Fetched page ${page}. ${allCardsForSet.length} of ${apiTotalCount} cards for this set.`
        );
    ...
    for (let i = 0; i < allCardsForSet.length; i += BATCH_SIZE) {
      ...
      chunk.forEach((card) => {
        if (card && card.id && card.tcgplayer?.prices) {
          // Added check for prices
    ...
    logs.push(
      `✅ Successfully updated prices and saved history for applicable cards in set '${setId}'.`
    );
