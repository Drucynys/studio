...
async function runFullDataResync(baseUrl: string) {
  ...
  try {
    ...
    if (!setsToSyncResponse.ok)
      throw new Error(`Failed to fetch set list for card sync: ${setsToSyncResponse.statusText}`);
...
export async function GET(request: Request) {
  ...
  if (!CRON_SECRET) {
    return NextResponse.json(
      { status: 'error', message: 'CRON_SECRET environment variable is not set.' },
      { status: 500 }
    );
  }
  ...
  try {
    ...
    if (localSetsCount >= remoteSetsCount) {
      console.log(
        `Automated check: No new sets found. Local: ${localSetsCount}, Remote: ${remoteSetsCount}.`
      );
      return NextResponse.json({
        status: 'noop',
    ...
    console.log(
      `Automated check: New sets found! Local: ${localSetsCount}, Remote: ${remoteSetsCount}. Triggering full sync.`
    );
    ...
    return NextResponse.json({
      status: 'success',
      message: 'New sets found. Full data resynchronization triggered and completed successfully.',
    });
  } catch (error: any) {
