...
export default function FriendsPage() {
  ...
  const handleFollowToggle = async (targetUserId: string, action: 'follow' | 'unfollow') => {
    ...
    try {
      ...
      console.log(
        `[CLIENT] Successfully ${action}ed user ${targetUserId}. A notification should be triggered if 'follow' was the action.`
      );
    } catch (err: any) {
      ...
      <main className="flex-grow container mx-auto p-4 md:p-8 space-y-8">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Find other collectors and see what cards they have.</CardDescription>
          </CardHeader>
          <CardContent>
              ...
              return (
                <Card key={foundUser.uid} className="shadow-lg">
                  <CardHeader className="flex flex-row items-center justify-between">
                    ...
                    <Button
                      onClick={() =>
                        handleFollowToggle(foundUser.uid, isFollowing ? 'unfollow' : 'follow')
                      }
                      ...
                      {foundUser.collectionPreview.length > 0 ? (
                        <div className="flex gap-2 overflow-x-auto pb-2">
                          {foundUser.collectionPreview.map((card) => (
                            <div
                              key={card.id}
                              className="relative flex-shrink-0 w-24 h-32 rounded-md overflow-hidden bg-muted"
                              data-ai-hint="pokemon card front"
                            >
                              <Image
                                src={card.imageUrl || 'https://placehold.co/200x280.png'}
                                alt={card.name}
                                fill
                                sizes="96px"
            ...
            <CardContent>
              {loadingFollowing || loadingFollowedProfiles ? (
                ...
                <p className="text-muted-foreground text-sm">
                  You are not following anyone yet. Use the search above to find other collectors.
                </p>
              )}
            </CardContent>
          </Card>
