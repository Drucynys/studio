...
import { Loader2, ServerCrash, ArrowLeft, Target, Images, Search, Hash, Grid, List, ChevronDown } from "lucide-react";
...
const PokemonDetailPage = () => {
  ...
  const [sortBy, setSortBy] = useState<'default' | 'value-high' | 'value-low' | 'rarity'>('default');
  ...
  const fetchPageData = useCallback(async (pageNum = 1, loadMore = false) => {
    ...
    try {
        ...
        fetchPromises.push(fetch(`/api/cards/by-pokemon/${encodeURIComponent(pokemonName)}?page=${pageNum}&pageSize=${CARDS_PER_PAGE}`));
  ...
  useEffect(() => {
    ...
    let filteredData = cardsForPokemon.filter(card =>
    ...
    if (sortBy === 'value-high') {
      ...
      filteredData = [...filteredData].sort((a, b) => getRarityRank(b.rarity) - getRarityRank(a.rarity));
    }
  ...
  const selectedCardIndex = selectedApiCard ? filteredCards.findIndex(c => c.id === selectedApiCard.id) : -1;
    ...
    <div className="flex flex-col min-h-screen bg-background">
                    ...
                    <div className="flex flex-row items-center gap-4 min-w-0">
                        {pokemonData && pokemonData.sprite && (
                            <div className="relative w-16 h-16 shrink-0" data-ai-hint="pokemon sprite">
                                <Image src={pokemonData.sprite} alt={pokemonName} layout="fill" unoptimized className="object-contain" />
                            </div>
                        )}
                         ...
                         <div className="relative flex items-center justify-center h-14 w-14 flex-shrink-0">
                           <svg className="h-14 w-14 transform -rotate-90">
                             ...
                             <circle
                               ...
                               strokeDashoffset={2 * Math.PI * 23 - (pokemonCompletion.percentage / 100) * (2 * Math.PI * 23)}
                               strokeLinecap="round"
                               stroke="currentColor"
                               fill="transparent"
                               r="23"
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground">
                            #{card.number} {card.rarity && `• ${card.rarity}`} {card.set.name && `• ${card.set.name}`}
                          </p>
                        </div>
                      </div>
      ...
      {selectedApiCard && (
        <AddCardToCollectionDialog
          ...
          prevCardImageUrl={hasPrevCard ? (filteredCards[selectedCardIndex - 1].images.large || filteredCards[selectedCardIndex - 1].images.small) : null}
          nextCardImageUrl={hasNextCard ? (filteredCards[selectedCardIndex + 1].images.large || filteredCards[selectedCardIndex + 1].images.small) : null}
        />
      )}
