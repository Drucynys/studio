...
async function safeFetch(url: string, options?: RequestInit) {
  try {
    ...
    if (response.status === 404)
      throw new Error('Internal Route Not Found (404). Please verify API paths.');
    ...
    throw new Error(
      `Server returned status ${response.status} with non-JSON content: ${text.substring(0, 50)}...`
    );
  } catch (err: any) {
                ...
                >
                  {masterSyncStatus === 'in-progress' ? (
                    <>
                      ...
                      Syncing ({masterSyncProgress.toFixed(0)}%)...
                    </>
                  ) : (
                    'Start Full Resync'
                  )}
              ...
              {isCheckingStatus ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 divide-x text-center">
                  <div>
                    ...
                    <p className="text-3xl font-bold">{setCount ?? '-'}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-3xl font-bold">{pokedexCount ?? '-'}</p>
                  </div>
                </div>
              )}
              <div className="text-center">
