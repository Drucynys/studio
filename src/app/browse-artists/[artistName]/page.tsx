...
const ArtistDetailPage = async ({ params }: ArtistDetailPageProps) => {
  ...
  return <ArtistDetailPageClient artistName={artistName} />;
};

// Client component to handle state and effects
const ArtistDetailPageClient = ({ artistName }: { artistName: string }) => {
  ...
  const handleAddCardToCollection = (condition: string, valueForCollection: number, variant?: string, quantity: number = 1) => {
    ...
    const newCard: CollectionPokemonCard = {
      ...
      quantity,
      language: "English", // Assuming English for now, as artist search is universal
