...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const loginAsDemoGuest = useCallback(() => {
    ...
    toast({
      ...
      description:
        'Welcome! You are exploring the app with a temporary Guest Account. Changes will be saved locally in memory.',
    });
  }, [toast]);
  ...
  const handleAuthSuccess = useCallback(
    async (userCredential: UserCredential) => {
      ...
      try {
        ...
        const docSnap = (await Promise.race([getDoc(userDocRef), createTimeout(10000)])) as any;
        ...
        if (!docSnap.exists()) {
          ...
          if (!newUser.displayName) {
            try {
              await Promise.race([updateProfile(newUser, { displayName }), createTimeout(5000)]);
            } catch (authError) {
          ...
          await Promise.race([setDoc(userDocRef, userProfileData), createTimeout(10000)]);
  ...
  const updateUserPrivacySetting = useCallback(
