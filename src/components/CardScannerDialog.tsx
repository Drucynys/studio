    ...
    if (!tesseractWorkerRef.current) {
      try {
        ...
        toast({
          ...
          description: 'Could not load the OCR engine.',
        });
      }
    }
  }, [toast]);
        ...
        if (err instanceof Error && err.name === 'OverconstrainedError') {
          ...
          try {
            ...
            toast({
              ...
              description:
                'Using basic camera mode. Advanced features like zoom might be unavailable.',
            });
            return;
          } catch (fallbackErr) {
            console.error('Fallback camera access also failed:', fallbackErr);
            errorMessage =
              'Camera access denied. Please ensure permissions are enabled and no other app is using the camera.';
          }
  ...
  const handleZoom = async (direction: 'in' | 'out') => {
    ...
    try {
      ...
      toast({
        variant: 'destructive',
        title: 'Zoom Error',
        description: 'Could not change zoom level.',
      });
    }
  };
  ...
  const parseOcrText = (text: string): Partial<OcrScanOutput> => {
    ...
    if (cardNumber) {
      ...
      if (cardNumberLineIndex !== -1) {
        ...
        let match =
          textAfterNumber.trim().match(symbolRegex) || textBeforeNumber.trim().match(symbolRegex);
        if (match && match[1]) {
  ...
  const handleCaptureAndScan = async () => {
    if (
      ...
      toast({
        variant: 'destructive',
        title: 'Scan Error',
        description: 'Camera or OCR not ready.',
      });
      return;
    }
    setIsCapturing(true);
    ...
    try {
      ...
      const {
        data: { text },
      } = await worker.recognize(imageDataUri);
      ...
      toast({
        variant: 'destructive',
        title: 'OCR Error',
        description: e instanceof Error ? e.message : 'Failed to process image with OCR.',
      });
      ...
      <DialogContent className="sm:max-w-lg md:max-w-xl lg:max-w-2xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <ScanText className="h-6 w-6 text-primary" /> Card Scanner (OCR)
          </DialogTitle>
          <DialogDescription>
        ...
        <DialogFooter className="flex-col sm:flex-row gap-2">
          ...
          <Button
            ...
            disabled={
              !hasCameraPermission ||
              isCapturing ||
              (!!error && !isCapturing && hasCameraPermission === null)
            }
