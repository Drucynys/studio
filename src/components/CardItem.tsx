...
export function CardItem({ card }: CardItemProps) {
  return (
    <Card className="shadow-lg hover:shadow-xl transition-shadow duration-300 animate-card-appear">
      <CardHeader>
      ...
      <CardContent className="space-y-3">
        {card.imageUrl ? (
            <div className="relative aspect-[2.5/3.5] w-full rounded-md overflow-hidden mb-2">
                 <Image src={card.imageUrl} alt={card.name || card.cardNumber} width={250} height={350} className="object-contain w-full h-full" data-ai-hint="pokemon card" />
            </div>
        ) : (
