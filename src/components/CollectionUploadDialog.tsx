    ...
    reader.onload = (e) => {
      ...
      const data = rows
        .map((row) => {
          ...
          const values =
            row.match(/(".*?"|[^",\s]+)(?=\s*,|\s*$)/g)?.map((v) => v.trim().replace(/"/g, '')) ||
            row.split(',');
          const rowData: any = {};
          ...
          return {
            ...
            language:
              rowData.language === 'Japanese' ? ('Japanese' as const) : ('English' as const),
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Upload className="h-5 w-5" /> Upload Collection from CSV
          </DialogTitle>
          <DialogDescription>
            Import your collection by uploading a CSV file.
            <br />
            <b>Required columns:</b> <code className="bg-muted px-1 py-0.5 rounded">name</code>,{' '}
            <code className="bg-muted px-1 py-0.5 rounded">set</code>,{' '}
            <code className="bg-muted px-1 py-0.5 rounded">cardNumber</code>.
            <br />
            <b>Optional columns:</b> <code className="bg-muted px-1 py-0.5 rounded">quantity</code>,{' '}
            <code className="bg-muted px-1 py-0.5 rounded">variant</code>,{' '}
            <code className="bg-muted px-1 py-0.5 rounded">language</code>,{' '}
            <code className="bg-muted px-1 py-0.5 rounded">isFavorite</code>.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="py-4 space-y-4">
          ...
          {parsedData.length > 0 && !uploadResult && (
            <Alert>
              ...
              <AlertDescription>
                Found {parsedData.length} cards in your file. Click "Upload Collection" to start the
                import.
              </AlertDescription>
            </Alert>
          )}
          ...
          {uploadResult && (
            <Alert
              variant={uploadResult.notFoundCount > 0 ? 'default' : 'default'}
              className={uploadResult.notFoundCount === 0 ? 'border-green-300' : ''}
            >
              ...
              <AlertDescription>
                ...
                {uploadResult.notFoundCount > 0 && (
                  <>
                    <br />
                    Could not find {uploadResult.notFoundCount} cards.
                    <details className="text-xs mt-2">
                      ...
                      <ul className="list-disc pl-4 mt-1">
                        ...
                        {uploadResult.notFound.length > 5 && (
                          <li>...and {uploadResult.notFound.length - 5} more.</li>
                        )}
                      </ul>
                    </details>
                  </>
